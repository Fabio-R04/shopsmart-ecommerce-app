...
import { filterProducts, getProductColors, getSectionProducts } from "../features/product/productSlice";
...
function Section() {
    ...
    useEffect(() => {
        dispatch(getSectionProducts(sectionType as string));
        dispatch(getProductColors(sectionType as string));
    }, [sectionType]);

    useEffect(() => {
        if (sectionProducts) {
            const min: number = Math.min(...sectionProducts.map((product) => {
                return product.productPrice;
            }));
            const max: number = Math.max(...sectionProducts.map((product) => {
                return product.productPrice;
            }));
    ...
    return (
        <div className="main-container">
            <Navbar />
            {(loadingProductSection || loadingProductColors) ? (
                <Spinner />
            ) : (
                <div className="section">
                    <div className="section__heading">
                        <h1>
                            {toTitleCase(sectionType as string)}
                        </h1>
                        <p>
                            Section | {toTitleCase(sectionType as string)}
                        </p>
                    </div>
